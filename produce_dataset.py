...
def produce_netcdf(save_dir: str,
    ...
    pdsi_out_df = pd.read_csv(outlier_table)
    pdsi_out_df.index = pdsi_out_df['Cell']
    del pdsi_out_df['Cell']
    pdsi_out_df.index = pdsi_out_df.index.str.replace('c', '').astype(int)
    cell_df = cell_df.join(pdsi_out_df)
    ...
    new_nc.createVariable("pdsi_outliers", 'f4', ('lat', 'lon'), zlib=True, shuffle=True, fill_value=np.nan)
    ...
    new_nc['pdsi_outliers'].setncattr('long_name', "Cells with PDSI over +/-10 in 1948-2018")
    ...
    new_nc['me'].setncattr('long_name', "Mean Error of PDSI Modeled 1948-2018")
    new_nc['mse'].setncattr('long_name', "Mean Square Error of PDSI Modeled 1948-2018")
    new_nc['rmse'].setncattr('long_name', "Root Mean Square Error of PDSI Modeled 1948-2018")
    ...
    pdsi_out_array = np.array([np.nan] * y_size * x_size).reshape((y_size, x_size))
    ...
    for i, cell in enumerate(pdsi_ext_df.columns):
        try:
            ...
            pdsi_out_array[row.y_idx, row.x_idx] = row.outlier
    ...
    new_nc['pdsi_outliers'][:] = pdsi_out_array
...
outlier_table = '/Users/rchales/data/pdsi_data/PDSI_results_8_24/pdsi_outliers.csv'

produce_netcdf(save_dir, cell_table, error_table, stats_table, pdsi_org_table, pdsi_ext_table, pdsi_mod_table,
               pdsi_new_table, outlier_table)
